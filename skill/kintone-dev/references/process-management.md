...
```
lookup（查這個 App 能用的欄位/群組/組織代碼——每個 App 都不一樣，不用去後台一個個抄）
  → 設計需求單（不懂 JS 的使用者可用純中文問答表填，交給 AI 轉成 spec）
  → spec（精簡規格檔：簽核鏈 + 分歧條件 + 樣板政策，約 100~150 行）
...
**每個 App 的欄位代碼、群組代碼都不一樣**是這套流程最容易卡住的地方——手動去 kintone 後台「系統管理」一個個找容易抄錯代碼（尤其群組/組織代碼常有全形括號或編號後綴）。若使用者不確定代碼或沒寫過 JS，先用 lookup 工具查出可用代碼，再用問答式需求單收集設計意圖（誰處理、要不要駁回、白話講的觸發條件），由 AI 轉譯成 spec 檔——比直接要求使用者手寫規格檔語法更不容易出錯。
...
> 開發指南 repo 的 `tools/process-gen/` 有一套零相依 Node 實作（lookup、design-intake.template.md、spec.example.js、generate/validate/compare/apply 六個檔案）可直接參考；該目錄未隨 skill 打包，已安裝環境找不到屬正常。沒有現成工具時，依上述分層現寫一套也不難——重點是**規格與樣板分離**這個結構。
